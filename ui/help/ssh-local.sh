...
if [ "${gui_lang}" == "ger" ]; then
		...
		<div class="col">
			...
			<div class="ps-4">
				<div class="alert alert-danger" role="alert">
					<strong>Hinweis 1:</strong><br />Aktiviere im Vorfeld bitte den <strong>SSH Terminal-Dienst</strong>. Zum Aktivieren des SSH Terminal-Dienstes navigiere zu <strong>DSM-Hauptmenü > Systemsteuerung > Terminal & SNMP</strong> und wechsel dort in den Reiter <strong>> Terminal</strong>. Aktivere die Checkbox <strong>SSH-Dienst aktivieren</strong>. Als SSH-Port wird standardmäßig der Port 22 verwendet, welchen du bei Bedarf anpassen kannst. Achte bei einer Änderung des Ports jedoch darauf, das der SSH-Port später identisch mit dem SSH-Verschlüsselungsport von rsync ist, da ansonsten der Verbindungsaufbau bzw. die Ausführung des rsync Befehls scheitern wird.<br /><br /><strong>Hinweis 2:</strong><br />Aktiviere im Vorfeld bitte auch den <strong>rsync-Dienst</strong>. Zum <strong>aktivieren des rsync Dienstes</strong> navigiere zu <strong>DSM-Hauptmenü > Systemsteuerung > Dateidienste</strong> und wechsel dort in den Reiter <strong>> rsync</strong>. Aktivere die Checkbox <strong>rsync Dienst aktivieren</strong>. Als SSH-Verschlüsselungsport wird standardmäßig der Port 22 verwendet, welchen du bei Bedarf anpassen kannst. Achte bei einer Änderung des Ports jedoch darauf, das der SSH-Verschlüsselungsport später identisch mit dem SSH-Port ist, da ansonsten der Verbindungsaufbau bzw. die Ausführung des rsync Befehls scheitern.
				</div>
				<p class="mt-3">
	...
	<div class="row">
		<div class="col">
			...
			<div class="ps-4">
				<div class="alert alert-danger" role="alert">
					<strong> Note 1: </strong> <br />Please activate the <strong>SSH terminal service</strong> in advance. To activate the SSH terminal service, navigate to <strong>DSM main menu > Control Panel > Terminal & SNMP</strong> and switch to the <strong>> Terminal</strong> tab. Activate the checkbox <strong>Enable SSH service</strong>. By default, port 22 is used as the SSH port, which you can adjust if necessary. However, if you change the port, make sure that the SSH port is later identical to the SSH encryption port of rsync, otherwise the connection establishment or the execution of the rsync command will fail.<br /><br /><strong> Note 2:</strong><br />Please also activate the <strong>rsync service</strong> in advance. To <strong>activate the rsync service</strong> navigate to <strong>DSM main menu > Control Panel > File Services</strong> and switch to the <strong>>rsync</strong> tab. Activate the checkbox <strong>Enable rsync service</strong>. By default, port 22 is used as the SSH encryption port, which you can adjust if necessary. However, if you change the port, make sure that the SSH encryption port is later identical to the SSH port, otherwise the connection establishment or execution of the rsync command will fail.
				</div>
				<p class="mt-3">
