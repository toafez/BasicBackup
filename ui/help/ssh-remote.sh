...
if [ "${gui_lang}" == "ger" ]; then
	...
	<div class="row">
		<div class="col">
			...
			<div class="ps-4">
				<p>
					An dieser Stelle würde auch hier ein RSA-Key erzeugt werden und zwar nach dem selben Prinzip, wie du es zuvor bereits auf deiner lokalen Synology NAS durchgeführt hast. Für die Arbeit mit Basic Backup ist dieser Schritt jedoch irrelevant und wird daher nicht weiter beschrieben.
				</p>
			</div>
			<h4 class="mt-4">authorized_keys anlegen</h4>
			<div class="ps-4">
				<p>
					In der Datei "authorized_keys" wird im späteren Verlauf der öffentliche Schlüssel Ihrer lokalen Synology NAS sowie weiterer bekannter Remote Server eingetragen, um so einen passwortlosen Verbindungsaufbau zu ermöglichen. Da die Datei noch nicht existiert, wird diese nun durch Eingabe des nachfolgenden Befehles erstellt.
				</p>
	...
	<div class="row">
		<div class="col">
			...
			<div class="ps-4">
				<p>
					At this point, an RSA key would also be generated here, using the same principle as you have already done on your local Synology NAS before. However, this step is irrelevant for working with Basic Backup and is therefore not described further.
				</p>
			</div>
			<h4 class="mt-4">Create authorized_keys</h4>
			<div class="ps-4">
				<p>
					Later, the public key of your local Synology NAS and other known remote servers will be entered in the file "authorised_keys" to enable a password-free connection. Since the file does not yet exist, it is now created by entering the following command.
				</p>
				<pre class="shadow-none p-2 mb-1 bg-light rounded"><span class="text-success">user@RemoteServer</span>:<span class="text-primary">~</span># touch ~/.ssh/authorized_keys</pre>
			</div>
