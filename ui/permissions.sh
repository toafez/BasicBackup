...
function synogroupuser()
{
	...
	for i in ${userlist}; do
		if [[ "${query}" == "adduser" ]]; then
			...
			synologset1 sys err 0x11100000 "Package [${app}] an error occurred while editing the app permissions!"
			exit 1
		fi
	done
	...
	if [[ -z "${userexists}" && "${query}" == "adduser" ]]; then
		...
		synologset1 sys info 0x11100000 "Package [${app}] has successfully expanded app permissions!"
		...
		synologset1 sys info 0x11100000 "Package [${app}] has successfully revoked advanced app permissions!"
